...
export const DonutChart: React.FC<DonutChartProps> = ({
  ...
  const renderDonutSegments = () => {
    ...
    data.forEach((item, index) => {
      ...
      if (showLabels) {
        // Use a smaller radius for labels on mobile devices
        const labelRadius = windowWidth < 640 ? responsiveRadius + 5 : 
                          windowWidth < 768 ? responsiveRadius + 10 :
                          responsiveRadius + 20;
        ...
        if (windowWidth < 480) { // Very small mobile
        ...
        segments.push(
          ...
          >
            <text
              ...
              className={`${windowWidth < 640 ? 'text-[0.65rem]' : 'text-xs'} font-medium fill-gray-700 dark:fill-gray-300`}
            >
              {displayLabel}
      ...
      <div className="mt-4 grid gap-2 text-sm max-w-full overflow-hidden"
        ...
        {data.map((item, index) => {
          ...
          if (windowWidth < 480) { // Very small mobile
            ...
            displayLabel = displayLabel.slice(0, 8) + '...';
          }
          ...
          return (
            ...
            >
              ...
              {showValues && windowWidth >= 480 && (
                <span className="ml-1 text-gray-500 dark:text-gray-400 text-xs">
                  ({((item.value / total) * 100).toFixed(1)}%)
                </span>
              )}
              {showValues && windowWidth < 480 && (
                <span className="ml-1 text-gray-500 dark:text-gray-400 text-[0.65rem]">
  ...
  const renderCenterContent = () => {
    ...
    if (showTotal) {
      ...
      const fontSize = responsiveSize < 150 ? 'text-xs' : responsiveSize < 200 ? 'text-sm' : 'text-base';
  ...
  return (
    <div className={`flex flex-col items-center ${className}`}>
      ...
      {title && (
        <h3 className="text-base font-medium text-gray-700 dark:text-gray-300 mb-4">{title}</h3>
      )}
